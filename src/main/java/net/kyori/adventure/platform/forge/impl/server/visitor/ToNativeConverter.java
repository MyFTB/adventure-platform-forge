...
public class ToNativeConverter implements ComponentVisitor {
    ...
    public void accept(BlockNBTComponent c) {
        this.nativeComponent =
            handleStyleAndChildren(c, new NbtComponent.BlockNbtComponent(c.nbtPath(), c.interpret(), c.pos().asString(), Optional.empty()));
    }

    @Override
    public void accept(EntityNBTComponent c) {
        this.nativeComponent =
            handleStyleAndChildren(c, new NbtComponent.EntityNbtComponent(c.nbtPath(), c.interpret(), c.selector(), Optional.empty()));
    }
    ...
    public void accept(StorageNBTComponent c) {
        ...
        this.nativeComponent =
            handleStyleAndChildren(c, new NbtComponent.StorageNbtComponent(c.nbtPath(), c.interpret(), location, Optional.empty()));
    }
